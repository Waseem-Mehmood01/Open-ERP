...
function attempt_login_user($user_name, $password, $company_id, $superadmin) {
	...
	$is_logged = DB::queryFirstRow("SELECT * FROM ".DB_PREFIX."test_users u WHERE (u.`user_name` LIKE '".$user_name."' OR u.`user_email` LIKE '".$user_name."') AND u.`password`='".$password."' AND u.`company_id`='".$company_id."' AND u.`user_status`='active'");	
	if($is_logged){
	...
	$_SESSION['role_id'] = $is_logged['role_id'];
